...
import by.hut.flat.calendar.core.DBAdapter.tasks.Book;
...
public class DialogBook extends DialogThread{
	public static final String ACTIVITY_TAG = DialogBook.class.getName();	
	...
	public void implementTask(){
		if (Do.equals(ACTION_BOOK)){
			...
			if (maidens.length > 1){
				...
				mAsyncTaskManager.setupTask(new Book(context,FlatID,firstDate,lastDate,maidenID));
			}
			else {
				int maidenID = -1;
				mAsyncTaskManager.setupTask(new Book(context,FlatID,firstDate,lastDate,maidenID));
	...
	public static void show(Context context, String[] keys,String[] params){
		Intent newIntent = new Intent(context, DialogBook.class);
		newIntent.putExtra(ACTION_DO,ACTION_BOOK);
		newIntent.putExtra(ACTION_PROGRESS, ACTION_NO);
